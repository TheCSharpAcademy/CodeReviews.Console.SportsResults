...
public class MailService
{
    ...
    {
        IConfiguration emailConfig = new ConfigurationBuilder()
        .AddJsonFile("appSettings.json")
        .Build();

        var emailSettings = emailConfig.GetSection("EmailSettings");
        ...
        while (true)
        {
            ...
            string smtpAddress = emailSettings["SmtpAddress"];
            int portNumber = int.Parse(emailSettings["PortNumber"]);
            bool enableSSl = bool.Parse(emailSettings["EnableSsl"]);
            string emailFromAddress = emailSettings["EmailFromAddress"];
